...
class ConnectorComments {
	...
	synchronized List<Comment> jiraGetComments(String issueKey) throws IOException,
	        XmlPullParserException, Exception {

		if (issueKey == null) throw new Exception("Wrong arguments\nissueKey: "
		        + issueKey);
		...
		SoapSerializationEnvelope envelope = connector
		        .getResponseFromServer(getComments);
	...
	void jiraAddComment(String issueKey, Comment comment) throws IOException,
		...
		if (comment == null || issueKey == null) throw new Exception(
